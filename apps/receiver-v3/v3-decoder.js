...
function coarseCellLuma(imageData, cellX, cellY, rotation, cellSize = 4) {
  const x = cellX * cellSize;
  const y = cellY * cellSize;
  return luma(sampleRegion(imageData, x + 1, y + 1, x + cellSize - 1, y + cellSize - 1, rotation));
}

function scoreCoarseRotation(imageData, rotation, profile = V3_G64_S4_C4_RS) {
  const dark = [];
  const light = [];
  ...
  for (let x = 0; x < profile.gridSize; x += 2) {
    dark.push(coarseCellLuma(imageData, x, 0, rotation, cellSize));
    dark.push(coarseCellLuma(imageData, x, last, rotation, cellSize));
    if (x + 1 < profile.gridSize) {
      light.push(coarseCellLuma(imageData, x + 1, 0, rotation, cellSize));
      light.push(coarseCellLuma(imageData, x + 1, last, rotation, cellSize));
    }
  }
  for (let y = 2; y < last; y += 2) {
    dark.push(coarseCellLuma(imageData, 0, y, rotation, cellSize));
    dark.push(coarseCellLuma(imageData, last, y, rotation, cellSize));
    if (y + 1 < last) {
      light.push(coarseCellLuma(imageData, 0, y + 1, rotation, cellSize));
      light.push(coarseCellLuma(imageData, last, y + 1, rotation, cellSize));
    }
  }
  ...
  return {
    ...
    timingSeparation,
    signatureSeparation,
    score: timingSeparation + signatureSeparation * 1.35,
  };
}
...
export function detectV3Coarse(roiCanvas, profile = V3_G64_S4_C4_RS) {
  ...
  const results = [0, 90, 180, 270].map((rotation) => scoreCoarseRotation(imageData, rotation, profile));
  results.sort((a, b) => b.score - a.score);
  const best = results[0];
  return {
    ...best,
    isV3: best.timingSeparation > 42 && best.signatureSeparation > 48,
  };
}
...
function sampleTimingCell(imageData, cellX, cellY, rotation, dx, dy, profile) {
  ...
  const pad = profile.cellSize * 0.28;
...
function sampleChromaticBackground(imageData, cellX, cellY, rotation, phase, profile) {
  ...
  for (let y = 1; y < profile.cellSize - 1; y += 1) {
    for (let x = 1; x < profile.cellSize - 1; x += 1) {
      ...
      if (chroma > 30) candidates.push({ rgb, chroma });
    }
  }
  candidates.sort((a, b) => b.chroma - a.chroma);
  const selected = candidates.slice(0, 72);
...
function extractShapeFeature(imageData, cellX, cellY, rotation, phase, backgroundRgb, profile) {
  const baseX = cellX * profile.cellSize + phase.dx;
  const baseY = cellY * profile.cellSize + phase.dy;
  const module = profile.cellSize / 6;
  const values = [];
...
export function decodeV3Roi(roiCanvas, rotation = 0, profile = V3_G64_S4_C4_RS) {
  ...
  if (phase.separation < 55) throw new Error(`V3 timing contrast too low (${phase.separation.toFixed(1)})`);
  const signatureSeparation = scoreHighResSignature(imageData, rotation, phase, profile);
  if (signatureSeparation < 60) throw new Error(`V3 signature too weak (${signatureSeparation.toFixed(1)})`);
  ...
  const recovered = recoverV3PacketFromSymbols(symbols, profile);
  ...
  return {
    ...
    averageConfidence: (averageColorConfidence + averageShapeConfidence) / 2,
  };
}

// Useful in tests and debug tools.
