import { V2_S8_C32_R3, getDataCellCoordinates } from '../../packages/optical-codec/src/profiles.js';
import { joinS8C32Byte } from '../../packages/optical-codec/src/shapes.js';
...
function sampleCellBackground(imageData, cellX, cellY, rotation, profile = V2_S8_C32_R3) {
...
function buildColorCalibration(imageData, rotation, profile = V2_S8_C32_R3) {
...
function classifyColor(rgb, calibration) {
  ...
  return { ...best, secondDistance: second.distance, confidence };
}

function extractShapeFeature(imageData, cellX, cellY, rotation, profile = V2_S8_C32_R3) {
  ...
  // Sender glyph is a 5×5 module mask, each module occupying 2×2 logical px,
  // centered at offset 3 inside the 16×16 cell.
  for (let gy = 0; gy < 5; gy += 1) {
    for (let gx = 0; gx < 5; gx += 1) {
      const x0 = baseX + 3 + (gx * 2);
      const y0 = baseY + 3 + (gy * 2);
      const rgb = sampleLogicalRegion(
        imageData,
        x0,
        y0,
        x0 + 2,
...
function buildShapeCalibration(imageData, rotation, profile = V2_S8_C32_R3) {
  return Array.from({ length: profile.shapeCount }, (_, shapeId) => {
    ...
    return {
      shapeId,
      feature: extractShapeFeature(imageData, x, y, rotation, profile),
    };
  });
}
...
function sampleSolidCellLuma(imageData, cellX, cellY, rotation, profile = V2_S8_C32_R3) {
  const baseX = cellX * profile.cellSize;
  const baseY = cellY * profile.cellSize;
  return luma(sampleLogicalRegion(
    imageData,
    baseX + 4,
    baseY + 4,
    baseX + 12,
...
export function scoreV2Signature(imageData, rotation, profile = V2_S8_C32_R3) {
  const dark = [];
  const light = [];
  profile.profileSignatureBits.forEach((bit, index) => {
    const value = sampleSolidCellLuma(
      imageData,
      profile.profileSignatureStartX + index,
      profile.profileSignatureRow,
      rotation,
      profile,
    );
  ...
  return {
    separation: lightMean - darkMean,
    darkMean,
    lightMean,
  };
}

export function detectV2Profile(roiCanvas, profile = V2_S8_C32_R3) {
  ...
  const signature = scoreV2Signature(imageData, orientation.rotation, profile);
  return {
    isV2: signature.separation > 75 && signature.darkMean < 135 && signature.lightMean > 150,
    orientation,
    signature,
  };
}
...
function componentVote(copies, key, distanceKey) {
  ...
  if (count >= 2) {
    return { value, corrected: counts.size > 1, uncorrectable: false };
  }
  const best = copies.reduce((a, b) => (a[distanceKey] <= b[distanceKey] ? a : b));
  return { value: best[key], corrected: false, uncorrectable: true };
}

export function recoverS8C32Copies(classifications, profile = V2_S8_C32_R3) {
  if (classifications.length !== profile.encodedByteCapacity) {
    throw new RangeError('V2 classification count does not match profile');
  }
  const bytes = new Uint8Array(profile.dataByteCapacity);
  let shapeCorrections = 0;
  let colorCorrections = 0;
  let shapeUncorrectable = 0;
  let colorUncorrectable = 0;

  for (let i = 0; i < profile.dataByteCapacity; i += 1) {
    const copies = Array.from({ length: profile.repetition }, (_, copy) =>
      classifications[i + (copy * profile.dataByteCapacity)],
    );
    ...
    bytes[i] = joinS8C32Byte(shape.value, color.value);
  }
...
export function decodeS8C32Roi(roiCanvas, profile = V2_S8_C32_R3) {
  if (!(roiCanvas instanceof HTMLCanvasElement)) {
    throw new TypeError('decodeS8C32Roi expects a canvas');
  }
  ...
  const signature = scoreV2Signature(imageData, orientation.rotation, profile);
  if (signature.separation < 75) {
    throw new Error(`V2 profile signature too weak (${signature.separation.toFixed(1)})`);
  }
  ...
  coordinates.forEach(({ x, y }) => {
    ...
    const color = classifyColor(backgroundRgb, colorCalibration);
    const shapeFeature = extractShapeFeature(imageData, x, y, orientation.rotation, profile);
  ...
  const recovery = recoverS8C32Copies(classifications, profile);
  ...
  if (packetLength < 1 || packetLength > profile.maxPacketBytes) {
    throw new Error(`Invalid decoded V2 packet length ${packetLength}`);
  }
